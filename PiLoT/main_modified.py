...
from crop_test.crop_wgs84_google.crop_wgs84_google.dsm_valid import DSM_MIN_VALID_ELEV_M, valid_dsm_elevation_mask
...
def split_image_name(path_or_name):
    """
    返回:
        stem: 不帶副檔名的檔名，例如 1658229736.063320980
        ext : 副檔名，例如 .jpg
        base: 完整檔名，例如 1658229736.063320980.jpg
    """
    base = os.path.basename(path_or_name)
    stem, ext = os.path.splitext(base)
    return stem, ext.lower(), base


def numeric_name_key(path_or_name):
    """
    支援以下檔名排序:
    - 123.jpg
    - 123.456.jpg
    ...
    優先按數值排序，失敗就退化成字串排序。
    """
    stem, _, base = split_image_name(path_or_name)
...
class DualProcessTask:        
    def __init__(self, config, init_euler = None, init_trans = None, name = None):
        ...
        pre_path = "/media/amax/AE0E2AFD0E2ABE69/datasets/uavscene//output/test"
        if not os.path.exists(pre_path):
            os.makedirs(pre_path)
        else:
            shutil.rmtree(pre_path)
            os.makedirs(pre_path)
        depth_txt_folder = "/media/amax/AE0E2AFD0E2ABE69/datasets/uavscene/depth"
        ...
        img_path = os.path.join(folder_path, 'images', dataset_name, 'interval1')
        self.img_list = (
        ...
        self.img_list = sorted(self.img_list, key=numeric_name_key)
        self.frame_names = [os.path.basename(p) for p in self.img_list]
    ...
    def rendering_worker(self):
        ...
        _ra_valid = valid_dsm_elevation_mask(ray_area)
        if not np.any(_ra_valid):
            raise ValueError(
                f"ray_area({ref_npy_path}) 中无有效 DSM 高程；要求 > {DSM_MIN_VALID_ELEV_M} m 且排除 NoData（如 -9999/-32767）"
            )
        ray_area_minZ = float(np.nanmin(ray_area[_ra_valid]))
        print("ray_area_minZ (valid min elevation): ", ray_area_minZ)
        # 与 crop 内相同：整幅 ray_area → float64 只做一次，避免每帧 ~1s+ 全图拷贝/扫描（数值与逐帧 astype 一致）
        ...
        while True:
            ...
            if idx >= len(self.img_list):
                logging.warning(f"idx={idx} 超出 self.img_list 長度={len(self.img_list)}")
                break

            src_img_path = self.img_list[idx]
            src_stem, src_ext, src_base = split_image_name(src_img_path)
            name = src_stem
            ...
            color = crop_data['color']          # HWC, RGB, uint8
            points3d = crop_data['points3d']    # HWCx3, float32

            if color is None or points3d is None:
            ...
            padded_save_path = os.path.join(pad_path, src_base)
            ...
            if points_valid.shape[0] == 0:
                logging.error(f"裁剪结果无有效3D点(经过padding/mask后): frame={name} path={padded_save_path}")
                continue
            ...
            self.task_q.put((color, points_sampled, p2d_r, visible_r, euler, trans))
            idx += 1
