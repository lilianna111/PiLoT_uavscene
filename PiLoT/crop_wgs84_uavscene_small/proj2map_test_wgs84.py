...
def generate_ref_map_multi_ratios(query_intrinsics, query_poses, area_minZ, dsm_data, dsm_transform, dom_data, ref_rgb_path, ref_depth_path, debug, crop_vis_save_path=None, test_ratio=None):
    ...
    return data
def visualize_projected_points_on_full_dom(
    dom_data,
    dsm_transform,
    image_points,
    K,
    ...
    """
    把 query 的四个角点（以及可选中心点）投到整幅 DOM 上并保存可视化结果。
    dom_data: (3,H,W)
    image_points: [(u,v), ...]
    center_point: (u,v) or None
    """
    if dom_data.ndim != 3 or dom_data.shape[0] != 3:
        raise ValueError(f"dom_data shape should be (3,H,W), got {dom_data.shape}")

    vis = np.transpose(dom_data, (1, 2, 0)).copy()
    ...
    # 四个角点
    for i, uv in enumerate(image_points):
        xyz = pixel_to_world(K, pose_w2c, uv, target_z=target_z)
        row, col = geo_coords_to_dsm_index(xyz, dsm_transform)

        projected_pixels.append((col, row))
        world_points.append(xyz)

        cv2.circle(vis, (int(col), int(row)), point_radius, colors[i % len(colors)], -1)
        cv2.putText(
            vis,
            f"P{i}",
    ...
    for i in range(len(projected_pixels)):
        p1 = projected_pixels[i]
        p2 = projected_pixels[(i + 1) % len(projected_pixels)]
        cv2.line(
            vis,
            (int(p1[0]), int(p1[1])),
            (int(p2[0]), int(p2[1])),
    ...
    if center_point is not None:
        center_world = pixel_to_world(K, pose_w2c, center_point, target_z=target_z)
        center_row, center_col = geo_coords_to_dsm_index(center_world, dsm_transform)
        center_pixel = (center_col, center_row)

        cv2.circle(vis, (int(center_col), int(center_row)), point_radius + 4, (255, 0, 255), -1)
        cv2.putText(
            vis,
            "C",
            (int(center_col) + 10, int(center_row) - 10),
            cv2.FONT_HERSHEY_SIMPLEX,
            font_scale,
            (255, 0, 255),
            thickness,
    ...
    print(f"    💾 已保存整图投影可视化: {save_path}")
    print("    四个角点在整图上的像素位置:")
    for i, (col, row) in enumerate(projected_pixels):
        print(f"      P{i}: col={col}, row={row}")
    if center_pixel is not None:
        print(f"      C : col={center_pixel[0]}, row={center_pixel[1]}")
...
def generate_ref_map(
    ...
    center_xyz = pixel_to_world(K_w2c, pose_w2c, center_point, target_z=area_minZ)
    center_row, center_col = geo_coords_to_dsm_index(center_xyz, dsm_transform)
    center_lon, center_lat, center_h = ecef_to_geodetic(center_xyz)

    print(
        f"C : uv={center_point}, col={center_col}, row={center_row}, "
        f"lon={center_lon:.12f}, lat={center_lat:.12f}, h={center_h:.3f}"
    )


    # ===== 在这里插入 =====
    full_dom_vis_path = None
    if crop_vis_save_path is not None:
        full_dom_vis_path = os.path.join(crop_vis_save_path, f"{name}_full_dom_points.jpg")
        visualize_projected_points_on_full_dom(
            dom_data=dom_data,
