...
class LearnedOptimizer(BaseOptimizer):
    ...
    def _run(self, p3D: Tensor, F_ref: Tensor, F_query: Tensor,
        ...
        # Keep runtime output compact by default.
        ...
        # print('优化前：',T_flat[0])
        err_list= []
        if hasattr(self.cost_fn, "begin_depth_debug_session"):
            self.cost_fn.begin_depth_debug_session(tag=f"iters={num_iters}")
        # Hard safety caps to prevent one bad linearization from exploding trajectory.
        rot_step_max = np.deg2rad(2.0)
        trans_step_max = 2.0
        for i in range(num_iters): #self.conf.num_iters
            ...
            dw_norm = torch.norm(dw, dim=-1, keepdim=True).clamp_min(1e-12)
            dt_norm = torch.norm(dt, dim=-1, keepdim=True).clamp_min(1e-12)
            dw = dw * torch.clamp(rot_step_max / dw_norm, max=1.0)
            dt = dt * torch.clamp(trans_step_max / dt_norm, max=1.0)
        ...
        if hasattr(self.cost_fn, "end_depth_debug_session"):
            self.cost_fn.end_depth_debug_session(print_summary=True)
