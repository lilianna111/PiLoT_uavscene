...
def get_3D_samples_v3(mkpts_r, depth_mat, T_c2w, camera, euler_angles, translation,  query_euler_angles, query_translation, last_frame_info = {}, origin = None, device = 'cuda', num_init_pose = 32, T_query_in_ECEF_c2w = None, mul = None):
    ...
    if use_precomputed_points:
        ...
        # [关键修改 3] 同步过滤 2D 点 (mkpts_r) !!!
        # 只有这里也过滤了，才能保证 499 对 499
        if mkpts_r is not None:
             # 1. 直接把前面乱七八糟的维度压扁，只保留点数(N)和坐标(2)
             # [1, 1, 500, 2] -> [500, 2]
             mkpts_r = mkpts_r.view(-1, 2)
             
             # 2. 现在是一维对应一维，直接过滤
             mkpts_r = mkpts_r[valid]
        ...
        # else:
        #     # 坐标转换: EPSG:4547 (投影坐标) -> EPSG:4978 (ECEF)
        #     # 注意：pyproj 运行在 CPU 上，需要先转 numpy
        #     transformer_4547_to_4978 = pyproj.Transformer.from_crs("EPSG:4547", "EPSG:4978", always_xy=True)
        #     x_np = points3d[:, 0].detach().cpu().numpy()
        #     y_np = points3d[:, 1].detach().cpu().numpy()
        #     z_np = points3d[:, 2].detach().cpu().numpy()
            
        #     X, Y, Z = transformer_4547_to_4978.transform(x_np, y_np, z_np)
        #     Points_3D_ECEF = torch.tensor(np.stack([X, Y, Z], axis=1), device=device, dtype=torch.float32)
        if points3d.numel() == 0:
            print("Warning: No valid 3D points in get_3D_samples_v3")
            Points_3D_ECEF = torch.empty((0, 3), device=device)
        else:
            # depth_mat / npy 已经是 ECEF (EPSG:4978)，这里不能再做坐标转换
            Points_3D_ECEF = points3d

            # 调试打印：确认数值尺度确实像 ECEF
            # print("[DEBUG get_3D_samples_v3] Using precomputed ECEF points directly")
            # print(f"[DEBUG get_3D_samples_v3] Points_3D_ECEF shape = {Points_3D_ECEF.shape}")
            # print(f"[DEBUG get_3D_samples_v3] Points_3D_ECEF min = {Points_3D_ECEF.min(dim=0).values.detach().cpu().numpy()}")
            # print(f"[DEBUG get_3D_samples_v3] Points_3D_ECEF max = {Points_3D_ECEF.max(dim=0).values.detach().cpu().numpy()}")
